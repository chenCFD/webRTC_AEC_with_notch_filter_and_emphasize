...
fid=fopen('mic1-16k.pcm', 'rb'); % Load far end 
ssin=fread(fid,inf,'int16');
fclose(fid);
%far is speaker played music
fid=fopen('ref-16k.pcm', 'rb'); % Load fnear end 
...
%     end
    ...
    %notch and preemph
    [dk,notch_mem]=filter_dc_notch16(dk,notch_mem,notch_radius,N);
    for i=1:N
        tmp32=dk(i)-dot(preemph,memd);
        memd=dk(i);
        dk(i)=tmp32;
    end
    for i=1:N
        tmp32=xk(i)-dot(preemph,memx);
        memx=xk(i);
        xk(i)=tmp32;
    end
...
fidout=fopen('out.pcm','wb');
fwrite(fidout,out,'int16');
fclose(fidout);    
fidaecout=fopen('aecout.pcm','wb');
fwrite(fidaecout,aecout,'int16');
fclose(fidaecout);
hold off

function [out,mem]=filter_dc_notch16(mic,mem,notch_radius,N)
    sz = size(mic);
    out = zeros(sz);
    den2 = notch_radius*notch_radius + 0.7 * (1-notch_radius)*(1 - notch_radius);
    for i=1:N
        vin=mic(i);
        vout=mem(1)+vin;
        mem(1) = mem(2) + 2*(-vin + notch_radius*vout);
        mem(2) = vin - (den2*vout);
        out(i) = notch_radius * vout;
    end
end
